...
Last Updated: 2026-03-20 (server-side-pagination-and-search)
...
| `AdminDeviceListQuery` | `ImpiTrack.Application.Abstractions` | `Application` | Paged query params for admin device list: page, pageSize, sortBy, sortDirection, search |
| `MeDeviceListQuery` | `ImpiTrack.Application.Abstractions` | `Application` | Paged query params for ME device list: page, pageSize, search |
| `OpsRawListQuery` | `ImpiTrack.Ops` | `Ops` | Paged query params for raw packet ops: page, pageSize, imei filter |
| `OpsSessionListQuery` | `ImpiTrack.Ops` | `Ops` | Paged query params for active sessions ops: page, pageSize, port filter |
| `OpsErrorListQuery` | `ImpiTrack.Ops` | `Ops` | Paged query params for error aggregates ops: page, pageSize, from/to range, groupBy |
...
### 6.11 Server-Side Pagination And Search

All list endpoints that return collections of records use server-side pagination via `PagedResult<T>`. No flat-array responses exist on paginated endpoints.

**Affected endpoints:**

| Endpoint | Query params | Sort |
|---|---|---|
| `GET /api/admin/users/{userId}/devices` | `page`, `pageSize`, `sortBy` (imei/boundAtUtc/alias), `sortDirection` (asc/desc), `search` (IMEI or alias) | whitelist-validated; 400 on unknown column |
| `GET /api/me/devices` | `page`, `pageSize`, `search` (IMEI or alias) | fixed: `bound_at_utc DESC` |
| `GET /api/ops/raw/latest` | `page`, `pageSize`, `imei` | fixed: `received_at_utc DESC` |
| `GET /api/ops/sessions/active` | `page`, `pageSize`, `port` | fixed: `last_seen_at_utc DESC` |
| `GET /api/ops/errors/top` | `page`, `pageSize`, `from`, `to`, `groupBy` (protocol/port/errorCode) | fixed: count DESC |

**Allowed page sizes:** `10`, `20`, `50`, `100`. A `400 invalid_page_size` is returned for any other value.

**Search filter (device endpoints):** `search` parameter performs a case-insensitive partial match (`LIKE @Search% + @Search` pattern) against both IMEI and alias columns. Uses `LIKE` on SQL Server and `ILIKE` on PostgreSQL.

**SQL pattern:** Two-query approach — `COUNT(*)` then paged `SELECT` with `OFFSET`/`FETCH` (SQL Server) or `LIMIT`/`OFFSET` (PostgreSQL). Exception: `errors/top` uses a single CTE with `COUNT(*) OVER()` to avoid a second round-trip on an already-aggregated query.

**`PagedResult<T>`** wrapper (defined in `Application.Abstractions`): `Items`, `Page`, `PageSize`, `TotalItems`, `TotalPages`.

**InMemory implementation** (`InMemoryDataRepository`): all paged methods use LINQ `Skip`/`Take` + `Where` for search, preserving identical semantics in tests.

### 6.12 TCP Integration Test Race Condition — WaitForPortAsync

`host.StartAsync()` returns as soon as the `IHostedService.StartAsync` completes, which for `BackgroundService` implementations returns after the first iteration is scheduled — before the TCP `Worker` binds the port. On fast CI runners this causes `Connection refused` on the immediately following `ConnectAsync`.

**Fix:** All four `TcpServerIntegrationTests` call `await WaitForPortAsync(port)` after `host.StartAsync()`. The helper polls TCP every 50 ms (max 5 s) until a probe connection succeeds, then returns. A `TimeoutException` is thrown if the port never opens.
...
private static async Task WaitForPortAsync(int port, int timeoutMs = 5000)
{
    ...
    {
        try { using var probe = new TcpClient(); await probe.ConnectAsync(IPAddress.Loopback, port); return; }
...
This pattern is required for every in-process TCP server test that calls `host.StartAsync()` before connecting a client.
...
- `ImpiTrack/ImpiTrack.Application/Abstractions/UserAccountModels.cs` (AdminDeviceListQuery, MeDeviceListQuery)
- `ImpiTrack/ImpiTrack.Ops/Models.cs` (OpsRawListQuery, OpsSessionListQuery, OpsErrorListQuery)
- `ImpiTrack/ImpiTrack.DataAccess/Repositories/SqlDataRepository.cs` (paged SQL queries)
- `ImpiTrack/ImpiTrack.DataAccess/InMemory/InMemoryDataRepository.cs` (LINQ paged queries)
- `ImpiTrack/ImpiTrack.Api/Controllers/MeController.cs` (paginated device list + search)
- `ImpiTrack/ImpiTrack.Api/Controllers/AdminUsersController.cs` (paginated device list + search)
- `ImpiTrack/ImpiTrack.Api/Controllers/OpsController.cs` (paginated raw/sessions/errors)
- `ImpiTrack/ImpiTrack.Tests/TcpServerIntegrationTests.cs` (WaitForPortAsync race condition fix)
